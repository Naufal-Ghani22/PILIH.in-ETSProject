...
$top_kampus = [];
while ($r = mysqli_fetch_assoc($q_kampus)) $top_kampus[] = $r;
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        /* ── Slider kampus ── */
        .campus-track { display: flex; gap: 1.5rem; transition: transform 0.5s cubic-bezier(.4,0,.2,1); }
        .campus-slide { flex: 0 0 320px; transition: transform 0.4s, box-shadow 0.4s, opacity 0.4s; opacity: 0.6; transform: scale(0.93); }
        .campus-slide.active { opacity: 1; transform: scale(1.04); }

        /* ── Tutorial connector line ── */
        .step-connector { position:absolute; top:2.5rem; left:calc(50% + 2.5rem); right:0; height:2px;
    ...
    <main class="flex-grow relative">
        ...
        <section class="py-20 relative z-10">
            <div class="container mx-auto px-4 lg:px-12">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-10">
                    ...
                    <div class="reveal bg-white/60 backdrop-blur-lg p-8 rounded-[2rem] border border-white/60 shadow-sm hover:shadow-xl hover:-translate-y-2 transition duration-300">
                        <div class="w-14 h-14 bg-white border border-purple-100 shadow-sm text-purple-600 flex items-center justify-center rounded-2xl mb-6">
        ...
        <section class="py-24 bg-white relative overflow-hidden">
            ...
            <div class="absolute inset-0 -z-0 pointer-events-none">
                <div class="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-purple-50 rounded-full blur-3xl opacity-60"></div>
            </div>
            <div class="container mx-auto px-4 lg:px-12 relative z-10">
                <div class="text-center mb-16 reveal">
                    <span class="inline-block text-xs font-bold uppercase tracking-widest text-purple-600 bg-purple-50 px-4 py-2 rounded-full mb-4">Cara Kerja</span>
                    <h2 class="text-4xl font-extrabold text-slate-900 mb-4">Mulai dalam <span class="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-indigo-500">4 Langkah Mudah</span></h2>
                    <p class="text-slate-500 max-w-xl mx-auto">Dari mendaftar hingga mendapatkan roadmap karir lengkap — semuanya cepat dan gratis.</p>
                </div>

                <!-- Steps dengan connector line -->
                <div class="relative">
                    <!-- Garis horizontal penghubung (desktop) -->
                    <div class="hidden lg:block absolute top-10 left-[12.5%] right-[12.5%] h-0.5 bg-gradient-to-r from-purple-200 via-indigo-300 to-purple-200 z-0"></div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10">
                        ...
                        <div class="reveal flex flex-col items-center text-center group" style="transition-delay: <?= $i*100 ?>ms">
                            <!-- Lingkaran step number -->
                            <div class="relative mb-6">
                                <div class="w-20 h-20 rounded-full bg-gradient-to-br <?= $s['color'] ?> flex items-center justify-center shadow-xl group-hover:scale-110 transition duration-300">
                                    <span class="text-3xl"><?= $s['emoji'] ?></span>
                                </div>
                                <!-- Nomor kecil di pojok -->
                                <span class="absolute -top-1 -right-1 w-6 h-6 rounded-full bg-white border-2 border-purple-200 flex items-center justify-center text-xs font-black text-purple-600">
                                    <?= $s['no'] ?>
                                </span>
                            </div>
                            <!-- Konten -->
                ...
                <div class="text-center mt-16 reveal">
                    <a href="tes.php" class="inline-flex items-center gap-3 px-10 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold rounded-2xl shadow-xl shadow-purple-500/30 hover:-translate-y-1 hover:shadow-2xl transition duration-300">
        ...
        <section class="py-24 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 overflow-hidden">
            <div class="container mx-auto px-4 lg:px-12">
                <div class="text-center mb-14 reveal">
                    <span class="inline-block text-xs font-bold uppercase tracking-widest text-purple-400 bg-purple-900/40 px-4 py-2 rounded-full mb-4">Kampus Terbaik</span>
                    <h2 class="text-4xl font-extrabold text-white mb-4">Top Kampus <span class="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400">PILIH.in</span></h2>
                    <p class="text-slate-400 max-w-md mx-auto">Universitas-universitas ini memiliki program studi terlengkap dalam platform kami.</p>
                </div>
                ...
                <div class="relative" id="sliderContainer">
                    ...
                    <div class="overflow-hidden">
                        <div id="campusTrack" class="campus-track">
                            ...
                            <div class="campus-slide <?= $idx===0?'active':'' ?>" data-index="<?= $idx ?>">
                                <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-8 hover:bg-white/10 transition duration-300 h-full">
                                    <!-- Logo & nama -->
                                    <div class="flex items-center gap-4 mb-6">
                                        <div class="w-16 h-16 bg-white rounded-2xl flex items-center justify-center overflow-hidden shadow-lg flex-shrink-0">
                                            <?php if (!empty($k['logo_kampus'])): ?>
                                                <img src="img/<?= htmlspecialchars($k['logo_kampus']) ?>" alt="Logo <?= htmlspecialchars($k['nama_kampus']) ?>" class="w-full h-full object-contain p-1">
                                            <?php else: ?>
                                                <span class="text-2xl font-black text-purple-600"><?= strtoupper(substr($k['nama_kampus'],0,2)) ?></span>
                                            <?php endif; ?>
                                        </div>
                                        <div>
                                            <h3 class="font-bold text-white text-lg leading-tight"><?= htmlspecialchars($k['nama_kampus']) ?></h3>
                                            <p class="text-slate-400 text-sm">📍 <?= htmlspecialchars($k['lokasi']) ?></p>
                                        </div>
                                    </div>
                                    <!-- Badge akreditasi -->
                                    <div class="flex flex-wrap gap-2 mb-4">
                                        <span class="bg-purple-500/20 text-purple-300 border border-purple-500/30 text-xs font-bold px-3 py-1 rounded-full">
                                    ...
                                    <!-- Estimasi biaya -->
                                    <p class="text-slate-400 text-xs mb-6">💰 <?= htmlspecialchars($k['estimasi_biaya']) ?></p>
                                    <a href="kampus.php" class="block w-full text-center py-2.5 rounded-xl border border-white/20 text-white text-sm font-semibold hover:bg-white/10 transition">
                                        Lihat Detail →
                                    </a>
                                </div>
                            </div>
                    ...
                    <!-- Prev / Next arrows -->
                    <button onclick="prevSlide()" class="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition">
                        ‹
                    </button>
                    <button onclick="nextSlide()" class="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition">
                        ›
                    </button>
                </div>
                <?php else: ?>
        ...
        <section class="py-24 bg-white">
            <div class="container mx-auto px-4 lg:px-12 max-w-3xl">
                <div class="text-center mb-14 reveal">
                    <span class="inline-block text-xs font-bold uppercase tracking-widest text-purple-600 bg-purple-50 px-4 py-2 rounded-full mb-4">FAQ</span>
                    <h2 class="text-4xl font-extrabold text-slate-900 mb-4">Pertanyaan <span class="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-indigo-500">yang Sering Ditanya</span></h2>
                ...
                <div class="space-y-3 reveal">
                    ...
                    <div class="faq-item border border-slate-100 rounded-2xl overflow-hidden hover:border-purple-100 transition">
                        <button class="faq-trigger w-full flex items-center justify-between px-6 py-5 text-left" onclick="toggleFAQ(this)">
                            <span class="font-semibold text-slate-800 pr-4"><?= $faq['q'] ?></span>
                            <span class="faq-icon flex-shrink-0 w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 font-bold text-lg">+</span>
                        </button>
